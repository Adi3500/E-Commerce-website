...
<main class="main-container">
  ...
  <div class="container">
        ...
        <form action="" method="post" enctype="multipart/form-data">
          <input type="text" name="seller" placeholder="search by seller/user/ name" id=""/>
...
if(isset($_POST['search'])){
  $search=$_POST['seller'];
  $sql = "SELECT * FROM `order` where `sellername`='$search'  or `username`='$search'"; // Selecting all rows from delivered_product table
$result = mysqli_query($connect, $sql);
...
echo "<table class='w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
    <thead class='text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400'>
        <tr>
            <th scope='col' class='px-3 py-3'>Sr.no</th>
...
$sqll = "SELECT * FROM `custom_order` where `sellername`='$search'  or `username`='$search'" ; // Selecting all rows from delivered_product table
...
$sql = "SELECT * FROM `order`"; // Selecting all rows from delivered_product table
$result = mysqli_query($connect,$sql);
...
$sqll = "SELECT * FROM custom_order " ; // Selecting all rows from delivered_product table
    $results = mysqli_query($connect, $sqll);
while ($row = mysqli_fetch_array($results)) {
  $totalPrice += $row['price']; // Accumulate total price
    $totalQuantity += $row['quantity']; // Accumulate total quantity
...
echo "</tbody>
    <tfoot>
        ...
        <tr colspan='15'>
            <td >Total Quantity of ORDER products:-</td>
            <td>$totalQuantity</td>
        </tr>
    </tfoot>
</table>";
